...
export const displayFragmentShader = `
  ...
  vec2 getResponsiveUV(vec2 uv, vec2 textureSize) {
    if (textureSize.x < 1.0 || textureSize.y < 1.0) return uv;

    float viewportAspect = uResolution.x / uResolution.y;
    vec2 scaledSize;

    if (viewportAspect > 1.15) {
      float maxWidth = uResolution.x * 0.58;
      float maxHeight = uResolution.y * 0.96;
      float scale = min(maxWidth / textureSize.x, maxHeight / textureSize.y);
  ...
  float getUvMask(vec2 uv) {
    vec2 feather = vec2(0.002);
    vec2 lower = smoothstep(vec2(0.0), feather, uv);
    vec2 upper = smoothstep(vec2(0.0), feather, 1.0 - uv);
  ...
  void main() {
    ...
    vec2 topUV = getResponsiveUV(vUv, uTopTextureSize);
    vec2 bottomUV = getResponsiveUV(vUv, uBottomTextureSize);
    ...
    float imageMask = min(getUvMask(topUV), getUvMask(bottomUV));

    gl_FragColor = mix(vec4(1.0), finalColor, imageMask);
  }
`;
